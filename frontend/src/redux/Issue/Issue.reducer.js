...
const issueReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    // Fixed: Separate case for UPDATE_ISSUE_STATUS_SUCCESS
    case actionTypes.UPDATE_ISSUE_STATUS_SUCCESS:
      return {
        ...
        // Also update issueDetails if it's the same issue
        issueDetails: state.issueDetails && state.issueDetails.id === action.issue.id 
          ? action.issue 
          : state.issueDetails,
      };
